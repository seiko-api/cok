...
if (isLimit(sender)) return reply(dpa.limitend)
if (args.length == 0) return reply(`Example: ${prefix + command} https://youtu.be/7KrV0drYfz0`)
ini_url = args[0]
url = args.join(' ')
buf = await getBuffer(`https://dapuhy-api.herokuapp.com/api/socialmedia/ytmp3v2?url=${url}&apikey=${DapKey}`)
await dp.sendMessage(form, buf, video, { quoted: lol })
break
...
if (isBanned) return reply(dpa.wait)
if (isLimit(sender)) return reply(dpa.limitend)
if (args.length == 0) return reply(`Example: ${prefix + command} https://vm.tiktok.com/ZSJg1qQUb/`)
ini_url = args[0]
url = args.join(' ')
buf = await getBuffer(`https://dapuhy-api.herokuapp.com/api/socialmedia/tiktoknowm?url=${url}&apikey=${DapKey}`)
await dp.sendMessage(form, buf, video, { quoted: lol })
break
...
if (isLimit(sender)) return reply(dpa.limitend)
reply(dpa.wait)
ini_buffer = await getBuffer(`https://dapuhy-api.herokuapp.com/api/asupan/${command}?apikey=${DapKey}`)
await dp.sendMessage(from, ini_buffer, video, { quoted: freply, mimetype: Mimetype.mp4, filename: "asupan.mp4" })
break                                
//==========================================BATES NGAB==========================================\\
//STALKER MENU
				case 'igstalk':
